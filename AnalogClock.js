...
export default class AnalogClock extends Component {
  ...
  constructor(props) {
      ...
      // this.state = {
      //   micSec: ( d.getSeconds() + d.getMilliseconds()/1000 ) /60 * 360,
      //   sec: d.getSeconds() * 6,
  ...
  componentDidMount() {
    ...
    //   let d = new Date();
    //   this.setState({micSec: ( d.getSeconds() + d.getMilliseconds()/1000 ) /60 * 360});
    ...
    return (
      <View style={ this.clockFrame() }>
        ...
        <View style={ this.clockHolder() }>
          ...
          <View style={[ this.secondHandStyles(),
            {transform:[{ rotate: this.state.micSec + 'deg' },
            {translateY: -(this.props.secondHandOffset +
                           this.props.secondHandLength/2) }]}]}
          />
